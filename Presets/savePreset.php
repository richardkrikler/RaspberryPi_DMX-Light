...
try {
  $stmt = $DB->prepare('SELECT COUNT(color_id) AS equal_colors FROM colors WHERE (red = :red) AND (green = :green) AND (blue = :blue) AND (alpha = :alpha)');
  ...
  if ($stmt->execute()) {
    ...
    if ($equal_colors[0] == 0) {
      try {
        $stmt = $DB->prepare('INSERT INTO colors (red, green, blue, alpha) VALUES (:red, :green, :blue, :alpha)');
        $stmt->bindParam(':red', $color_obj->red, PDO::PARAM_INT);
        $stmt->bindParam(':green', $color_obj->green, PDO::PARAM_INT);
        $stmt->bindParam(':blue', $color_obj->blue, PDO::PARAM_INT);
        $stmt->bindParam(':alpha', $color_obj->alpha, PDO::PARAM_STR);
